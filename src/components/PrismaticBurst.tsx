...
// @ts-ignore - OGL types are not fully available
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    dataRef.current.paused = paused;
  }, [paused]);

  useEffect(() => {
    dataRef.current.hoverDamp = hoverDampness;
  }, [hoverDampness]);
  ...
  useEffect(() => {
    ...
    dataRef.current.gradTex = gradientTex;
    ...
    dataRef.current.program = program;

    // @ts-ignore
    const triangle = new Triangle(gl);
    // @ts-ignore
    const mesh = new Mesh(gl, { geometry: triangle, program });
    dataRef.current.tri = triangle;
    dataRef.current.mesh = mesh;
    ...
    const handlePointer = (e: Event) => {
      ...
      const x = (pe.clientX - rect.left) / Math.max(rect.width, 1);
      const y = (pe.clientY - rect.top) / Math.max(rect.height, 1);
      dataRef.current.mouseTarget = [Math.min(Math.max(x, 0), 1), Math.min(Math.max(y, 0), 1)];
    };
    ...
    if ('IntersectionObserver' in window) {
      io = new IntersectionObserver(
        entries => {
          if (entries[0]) {
            dataRef.current.isVisible = entries[0].isIntersecting;
          }
        },
    ...
    const update = (now: number) => {
      ...
      last = now;
      const visible = dataRef.current.isVisible && !document.hidden;
      if (!dataRef.current.paused) accumTime += dt;
      ...
      const tau = 0.02 + Math.max(0, Math.min(1, dataRef.current.hoverDamp)) * 0.5;
      const alpha = 1 - Math.exp(-dt / tau);
      const tgt = dataRef.current.mouseTarget;
      const sm = dataRef.current.mouseSmooth;
      ...
      renderer.render({ scene: dataRef.current.mesh });
      raf = requestAnimationFrame(update);
    };
    raf = requestAnimationFrame(update);
    ...
    return () => {
      ...
      ro?.disconnect();
      if (!ro) (window as Window).removeEventListener('resize', resize);
      io?.disconnect();
  ...
  }, [mixBlendMode, paused, hoverDampness]);

  useEffect(() => {
    const program = dataRef.current.program;
    const renderer = dataRef.current.renderer;
    const gradTex = dataRef.current.gradTex;
  ...
  return <div className="absolute inset-0 overflow-hidden" ref={containerRef} />;
};
