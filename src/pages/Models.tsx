...
import { Search, Grid, List, Filter, X, Copy, Play, Video, Trophy } from "lucide-react";
...
import { ModelsSidebar } from "@/components/sections/ModelsSidebar";
...
const providerLogos: Record<string, string> = {
  ...
  "Vertex": vertexaiLogo,
  "Grok": grokLogo,
  // Lowercase mappings for easier lookup
  "bytedance": bytedanceLogo,
  "flux": fluxLogo,
  "ideogram": ideogramLogo,
  "kling": klingLogo,
  ...
  "vertex": vertexaiLogo,
  "gemini": vertexaiLogo,
  "luma": runwayLogo, // Fallback or placeholder if no specific logo
  "minimax": replicateLogo, // Fallback
  "fal": replicateLogo, // Fallback
  "runware": fluxLogo, // Fallback
};
...
export default function Models() {
  ...
  const filteredAndSortedModels = useMemo(() => {
    ...
    if (filters.modelType.length > 0) {
      result = result.filter(model => {
        // Check if model matches ANY of the selected types
        return filters.modelType.some(type => {
          if (type === "text-to-image") return model.output.includes("image");
          if (type === "text-to-video") return model.output.includes("video");
          if (type === "image-to-image") return model.output.includes("image") && (model.supported_params.edit || model.supported_params.mask);
          if (type === "image-to-video") return model.output.includes("video") && model.supported_params.edit;
          return false;
        });
      });
    }
                ...
                <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
                  ...
                  <SheetContent side="left" className="w-80 p-0">
                    ...
                    <div className="p-4 overflow-y-auto">
                      {/* We'll add filter content here if needed */}
                    </div>
                  </SheetContent>
                </Sheet>
              </div>
